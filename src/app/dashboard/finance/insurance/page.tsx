...
import { ShieldCheck, Bot, FileText, CheckCircle, Info, ArrowRight } from "lucide-react";
  ...
  return (
    <div className="flex flex-1 flex-col gap-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold">{translate("Find Crop Insurance")}</h1>
          <p className="text-muted-foreground text-sm sm:text-base">{translate("Find relevant government insurance schemes for your crops.")}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        <Card className="lg:col-span-1">
          <CardHeader>
            <CardTitle>{translate("Find Insurance Schemes")}</CardTitle>
            <CardDescription>{translate("Enter your details to find relevant crop insurance options from our AI assistant.")}</CardDescription>
          </CardHeader>
          <CardContent>
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                ...
                />
                 <FormField
                    ...
                    render={({ field }) => (
                        <FormItem>
                        <FormLabel>{translate("Primary Crop Type")}</FormLabel>
                ...
                <Button type="submit" className="w-full" disabled={isLoading}>
                    ...
                    {isLoading ? translate('Finding Schemes...') : translate('Find Insurance Schemes')}
                </Button>
              </form>
            </Form>
          </CardContent>
        ...
        <div className="lg:col-span-2">
            {isLoading ? (
                 <div className="flex h-full items-center justify-center rounded-lg bg-muted/50 min-h-[400px]">
                    ...
                    <p className="mt-4 font-semibold">{translate("AI is searching for schemes...")}</p>
                  ...
                  {schemes.length > 0 ? (
                    schemes.map((scheme) => (
                      <Card key={scheme.name}>
                        <CardHeader>
                          <CardTitle className="flex items-center gap-2"><CheckCircle className="text-primary"/>{scheme.name}</CardTitle>
                        </CardHeader>
                     ...
                     <div className="flex h-full flex-col justify-center rounded-lg border-2 border-dashed bg-muted/50 p-8 min-h-[400px]">
                        <div className="flex items-center gap-4">
                            ...
                            <div className="text-muted-foreground">
                                <p className="font-semibold text-lg">{translate("No Schemes Found")}</p>
                                <p className="mt-1 text-sm">{translate("The AI could not find any specific schemes for the selected criteria.")}</p>
                            </div>
                        </div>
                    </div>
                  )}
                ...
                <div className="flex h-full flex-col justify-center rounded-lg border-2 border-dashed bg-muted/50 p-8 min-h-[400px]">
                    <div className="flex items-center gap-4">
                        ...
                        <div className="text-muted-foreground">
                            ...
                            <p className="mt-1 text-sm">{translate("Fill out the form to find relevant insurance schemes.")}</p>
                        </div>
                    </div>
                </div>
            )}
